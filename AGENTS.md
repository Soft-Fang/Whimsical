...
## 目录结构
- `content/`           所有 Markdown 内容的根目录（Obsidian vault 的根是**仓库根目录**，即含 `.obsidian/` 的那一层）
- `content/blog/`      公开文章（只放 published，专题：正文）
- `content/apps/`      应用介绍（专题：应用）
- `content/talk/`      随笔 / 碎碎念（专题：随笔）
...
- `src/lib/posts.ts`   公共查询：`getPublishedPosts()`（只取 published）、`coverURL()`（解析封面）
- `scripts/`           工具：encrypt.mjs（私有区加密）、check.mjs（内容体检，覆盖 blog/talk/apps + 专题归属 + 图片 + 泄密）
- `维护教程.md`        本地维护说明（**gitignore，不上传**）——**每次改动前先读它**，含各文件夹定性

## 内容规范（frontmatter）
blog 文章必填：`title`、`description`、`pubDate`（YYYY-MM-DD）。
- `category`：固定标签，按文件夹锁死（blog=正文、talk=随笔、apps=应用）；专题归属由「放在哪个文件夹」决定，勿手改。`npm run check` 会校验一致性
- `cover`：封面图（`covers/xxx.webp` 放 `public/covers/`，或完整外链）；留空则自动配图
- `tags`：数组（或单个字符串，schema 会自动转数组）
- `status`：draft / review / published / private，默认 published
  - **所有页面 / RSS 只渲染 published**（由 `src/lib/posts.ts` 统一过滤）；公开区 `content/blog/` 只放 published，草稿放 `content/drafts/`，私密放 `content/private/`；非 published 混进 blog/ 会被 check 报错
...
## 与 Obsidian 的配合
- 在 Obsidian 中把**仓库根目录**作为 vault 打开（`.obsidian/` 在仓库根，模板路径形如 `content/templates/...`）
